...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  if (!category) {
    return {
      title: 'カテゴリが見つかりません | Kota Murai Life & Code',
    }
  }
  ...
  const buildPageUrl = (page: number) => {
    if (page === 1) return `${BASE_URL}/blog/category/${slug}`
    return `${BASE_URL}/blog/category/${slug}?page=${page}`
  }

  // タイトルとdescription
  let title = `${category.name} | ブログ | Kota Murai Life & Code`
  let description = category.description || `${category.name}に関する記事一覧`

  if (currentPage > 1) {
    title = `${category.name} (${currentPage}ページ目) | ブログ | Kota Murai Life & Code`
    description = `${category.name}に関する記事一覧（${currentPage}ページ目）`
  }
...
export default async function CategoryPage({ params, searchParams }: Props) {
  ...
  if (currentPage > totalPages && totalPages > 0) {
    redirect(`/blog/category/${slug}?page=${totalPages}`)
  }
